...
% Load structure with overall details per subject
load EyeSound
...
% This is the pilot! In the real experiment, we want to alternate between
% active and passive blocks somehow. Now, we will only do active
% acquisition. We will do 6 blocks within one contingency, which is what we
% will also do in the real experiment, but we will only do 2 contingencies,
% to keep it short (real experiment: 10)

condition = 1; % Active condition
nBlocks = 6; % This means you get 6 exlore trials per contingency
nContingencies = 2; % For pilot; in real experiment, there are 10
...
black = [0 0 0];
red = [1 0 0];
% window_size = [0 0 800 600]; % small window for debugging; comment out if fullscreen is wanted
...
% pixelXScale = screenXpixels / (dim * 2 + 2);
...
speed = 10; % This is how fast animation moves
...
explorelogfilename = [Results_path sprintf('%02d',iSub) 'explore.txt'];
...
testlogfilename = [Results_path sprintf('%02d', iSub) 'test.txt'];
...
headers={'Subject ', 'Contingency ', 'Block ', 'Trial ', 'Condition ', 'Sound ', 'MovDir ', 'Congruency ', 'Response '}; % Add one row of headers to the logfile
...
paHandle = PsychPortAudio('Open', device, [], 0, FS, nrchannels); % Open the Audio port and get a handle to refer to it in subsequent calls
    ...
    for iTest = 1:EyeSound(iSub).nTestTrialsperBlock % Loop over the 6 test trials of a block
        fprintf(LOGFILEtest,'\n%d', iSub);
        fprintf(LOGFILEtest,'\t%d', iContingency);
