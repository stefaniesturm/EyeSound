...
fixation = 0;
t = timer('TimerFcn', 'stat=false; fixation = 1',... 
                 'StartDelay',fixateTime);
...
% Some preparations for showing an image of the possible movement
% moveDirectionections
% 
% % Here we load in an image from file
% imageFile = imread(imageLoc);
% 
% % Make the image into a texture
% imageTexture = Screen('MakeTexture', window, imageFile);
% 
% % Get the size of the image
% [s1, s2, ~] = size(imageFile);
% 
...
% % height
% heightScalers = linspace(1, 0.7, 10);
...
%     Eyelink('command', 'calibration_type = HV9');
    ...
    % calibration/drift correction target (probably not necessary)
...
% Screen('Flip',window);
% if nano_exist == 1
%     DrawFormattedText(window, 'Pulse una flecha para comenzar.', 'center', 'center');
%     Screen('Flip', window);
...
%         WaitSecs(0.05)
%     end
%     %     Screen('Flip', window); % This should turn the window black
% end
...
for iBlock = 1:nBlocks
    ...
    %             % Get a keyboard press from the participant to continue for good
    %             if nano_exist == 1
    %                 [key,time,messages,Ts] = getmidiresp(); % keep checking for response
    %                 if ~isempty(key) % if a response key is pressed
    %                     DrawFormattedText(window, 'Presiona una de las flechas para seguir.', 'center', 'center', white);
    %                     Screen('Flip',window);
    %                 end
    %             end
end % Blocks
