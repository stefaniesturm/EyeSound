function EyeSound_Active_EL_triggers(iSub,StartContingency,StartBlock)
...
for iContingency = StartContingency:nContingencies
    ...
    for iBlock = StartBlock:nBlocks
        ...
        porttalk(hex2dec('CFB8'), 010, 1000) % trigger for active acquisition trial start
        Screen('Flip',window);
        WaitSecs(1);
        Screen('Flip',window);
            ...
            if Eyelink( 'NewFloatSampleAvailable') > 0
                ...
                if eye_used ~= -1 % Do we know which eye to use yet?
                    ...
                    if xEye~=el.MISSING_DATA && yEye~=el.MISSING_DATA && evt.pa(eye_used+1)>0
                        ...
                        still = isequal(currentEye, [xEye yEye]);
                        ...
                        if laststop ~= prevstop
                            ...
                            if dir ~= -1
                                ...
                                tone = dir
                                if iBlock == 1
                                    trigger = 011; % 0 means acquisition, 1 means active, 1 is block
                                elseif iBlock == 2
                                    trigger = 012;
                                elseif iBlock == 3
                                    trigger = 013;
                                ...
                                porttalk(hex2dec('CFB8'), trigger, 1000) % trigger for sound that was played (acquisition, active)
                            end
                        end
                        Screen('Flip', window);
                    else
        ...
        for iTest = 1:EyeSound(iSub).nTestTrialsperBlock % Loop over the 6 test trials of a block
            ...
            porttalk(hex2dec('CFB8'), 255, 1000) % trigger for start of test trial
            ...
            if testtrials(1,iTest) == 1 % If the direction of movement is 1
                ...
                switch animation
                    case 1
                        ...
                        trigger = [num2str(testtrials(3,iTest)) '1' num2str(iTest)]; % Make a trigger based on type of sound (congruent or incongruent), 1 for active condition, and number of trial (1-6)
                        porttalk(hex2dec('CFB8'), trigger, 1000)
                        WaitSecs(1);
