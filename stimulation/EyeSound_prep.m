function EyeSound_prep(nSubjects)

% This is based on the setup script that Iria wrote. It includes only that
% stuff that is really necessary for the EyeSound_run script. It uses
% different syllables to avoid confusion due to similar sounds. It
% generates a file called "EyeSound_data" which is needed to generate the
% syllables (see folder "sound-generation") and to execute EyeSound_run

%% Setup overall parameters for task organization
nContingencies = 20; % 14 + 1 (training), plus some backup buffer, in case we have to repeat levels!
nBlocksPerContingency = 7; % 7 blocks per contingency
nTestTrialsperBlock = 6; % 6 test trials per block
nMovementDirs = 8;
...
for iSub = 1:nSubjects
    ...
    % Save to EyeSound_data structure
    EyeSound_data(iSub).Counterbalancing = Counterbalancing;% 1 = start with Active; 2 = start with Passive
    ...
    % Save to EyeSound_data structure
    EyeSound_data(iSub).Contingencies = Contingencies;% array with the wavfile names of the nContingencies x nExemplars corresponding to each movement direction in each contingency block
    ...
    for iContingency = 1:nContingencies
        for iBlockperCont = 1:nBlocksPerContingency
            ...
            % Save into EyeSound_data structure
            EyeSound_data(iSub).Blocks(iBlock).TestTrials = TestTrialsBlock;
...
save('EyeSound_data.mat','EyeSound_data')
