        ...
        const mxArray *ins[])
{
  ...
  if (number_of_ins > 0)
  {
    ...
    if (!strcmp(command, "open"))
    {
      ...
      // Finally, we're ready to open what the user provided or selected
      // via input number or string.
      bool success = false;
      ...
      {
        mexPrintf("Device selection was cancelled or invalid.\n");
      }
    ...
    if (number_of_outs >= 1)
    {
      ...
      for (unsigned int k = 0; k < number_messages_stored; ++k)
      {
        ...
        *(messages_out + 3*k + 2) = (double)(message_buffer[read_index][2]);
        ...
        // Write out the timestands with the messages.
        if (number_of_outs == 2)
        {
          *(message_times_out + k)  = message_time_buffer[read_index];
        }
